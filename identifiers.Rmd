...
1. Create a data frame, with the following named columns:
...
the newest `vTagID`. I lastly added a `"River"` column by referencing the `allSites`
data frame.
...
write_csv(PITs_w_vTag, 'PITs_w_vTag.csv')
```
...
write_csv(dropped_vTagIDs, 'dropped_vTagIDs.csv')
```



# Master data frame of observations
...
In creating the `valid_PIT` data frame, I'm wrapping the `PIT_to_vTagID` function inside
the `Vectorize` function so that the new "vectorized" function uses and returns a
vector. I could have also used `sapply` as I did above, but `Vectorize` was simpler in 
this case, since I'd already created the base function.
For more information on vectorization, see
[here](http://www.noamross.net/blog/2014/4/16/vectorization-in-r--why.html).
...
```{r save_masterObs, eval = FALSE}
write_csv(masterObs, 'masterObs.csv')
```
