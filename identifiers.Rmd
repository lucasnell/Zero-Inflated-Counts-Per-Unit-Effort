...
The max appears to be 3 for both tables above, so I'll create an object to store this 
value for later.
```{r maxUniques}
```
...
I will now expand the lookup data frame to account for the fact that some individuals
are represented by multiple `vTagID`s.
