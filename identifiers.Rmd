...
We need the `allSites` data frame created in the compiling stage. The `rm` command 
...
In creating the second data frame (with `NA` vTagIDs but valid PIT tags), I'm wrapping 
the `PIT_to_vTagID` function inside the `Vectorize` function so that the new 
"vectorized" function uses and returns a vector. 
I could have also used `sapply` as I did above, but `Vectorize` was simpler in 
